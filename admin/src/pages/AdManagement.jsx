...
export default function AdManagement() {
  ...
  const handleToggleMainOncePerDay = () => {
    if (!adSettings.global_enabled) return;
    const newVal = !adSettings.main_ad_once_per_day;
    applySettingsUpdate({ main_ad_once_per_day: newVal });
  };

  const handleToggleSecondaryOncePerDay = () => {
    if (!adSettings.global_enabled) return;
    const newVal = !adSettings.secondary_ad_once_per_day;
    applySettingsUpdate({ secondary_ad_once_per_day: newVal });
  };
    ...
    <div>
      ...
      <div className="p-6 mb-6 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-sm">
        <div className="flex flex-wrap items-center gap-6">
          ...
          {/* 投放控制：总开关/主/次 */}
          <div className="flex flex-wrap items-center gap-4 px-4 py-3 rounded-lg bg-gray-50 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-700">
            ...
            <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">频率控制</span>
            <div className="flex items-center gap-2">
              <span className={`text-sm ${adSettings.global_enabled ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}`}>主广告每日一次</span>
              <label className={`relative inline-flex items-center ${adSettings.global_enabled ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}>
                <input
                  type="checkbox"
                  checked={!!adSettings.main_ad_once_per_day}
                  onChange={handleToggleMainOncePerDay}
                  className="sr-only peer"
                  disabled={!adSettings.global_enabled || settingsLoading}
                />
                <div className="w-11 h-6 bg-gray-200 dark:bg-gray-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
            ...
            <div className="flex items-center gap-2">
              <span className={`text-sm ${adSettings.global_enabled ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}`}>次要广告每日一次</span>
              <label className={`relative inline-flex items-center ${adSettings.global_enabled ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}>
                <input
                  type="checkbox"
                  checked={!!adSettings.secondary_ad_once_per_day}
                  onChange={handleToggleSecondaryOncePerDay}
                  className="sr-only peer"
                  disabled={!adSettings.global_enabled || settingsLoading}
                />
                <div className="w-11 h-6 bg-gray-200 dark:bg-gray-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
